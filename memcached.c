...
static void complete_nread_ascii(conn *c) {
	printf("ke complete_nread_ascii\n");
  assert(c != NULL);
...
/* Form and send a response to a command over the binary protocol */ static void write_bin_response(conn *c, void *d, int hlen, int keylen, int dlen) { printf("ke write_bin_response\n"); if (!c->noreply || c->cmd == PROTOCOL_BINARY_CMD_GET || c->cmd == PROTOCOL_BINARY_CMD_GETK) { add_bin_header(c, 0, hlen, keylen, dlen); if(dlen > 0) { add_iov(c, d, dlen); } conn_set_state(c, conn_mwrite); c->write_and_go = conn_new_cmd; } else { conn_set_state(c, conn_new_cmd); } }

static void complete_incr_bin(conn *c) {
	printf("ke complete_incr_bin\n");
  item *it;
  char *key;
  size_t nkey;
...
static void process_bin_get_or_touch(conn *c) {
	printf("process_bin_get_or_touch\n");
  item *it;
...
static void handle_binary_protocol_error(conn *c) {
	printf("ke handle_binary_protocol_error\n");
...
static void process_bin_sasl_auth(conn *c) {
	printf("ke process_bin_sasl_auth\n");
...
static void process_bin_complete_sasl_auth(conn *c) {
	printf("ke process_bin_complete_sasl_auth\n");
  assert(settings.sasl);
  const char *out = NULL;
...
static void dispatch_bin_command(conn *c) {
	printf("ke dispatch_bin_command\n");
...
static void process_bin_append_prepend(conn *c) {
	printf("ke process_bin_append_prepend\n");
  char *key;
  int nkey;
  int vlen;
  item *it;
...
static void complete_nread_binary(conn *c) {
	printf("ke complete_nread_binary\n");
    assert(c != NULL);
    assert(c->cmd >= 0);
...
inline static void process_stats_detail(conn *c, const char *command) {
	printf("ke process_stats_detail\n");
    assert(c != NULL);
...
static void process_stat_settings(ADD_STAT add_stats, void *c) {
	printf("ke process_stat_settings\n");
    assert(add_stats);
...
static void process_touch_command(conn *c, token_t *tokens, const size_t ntokens) {
	printf("ke process_touch_command\n");
    char *key;
    size_t nkey;
    int32_t exptime_int = 0;
    item *it;
...
static void process_arithmetic_command(conn *c, token_t *tokens, const size_t ntokens, const bool incr) {
	printf("ke process_arithmetic_command\n");
...
static void process_verbosity_command(conn *c, token_t *tokens, const size_t ntokens) {
	printf("process_verbosity_command\n");
    unsigned int level;
...
static void process_slabs_automove_command(conn *c, token_t *tokens, const size_t ntokens) {
	printf("process_slabs_automove_command\n");
    unsigned int level;
